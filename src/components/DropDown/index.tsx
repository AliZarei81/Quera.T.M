...
const Dropdown: React.FC = () => {
  ...
  const handleMouseEnter = (index: number, type: 'lessson' | 'project', secondindex?: number) => {
    switch (type) {
      case 'lessson':
      ...
      {isOpen &&
        <div className=' flex flex-col gap-[16px] mt-[16px]'>
          ...
          {items.map((item, index) => (
            ...
            <div key={index} className=' inline-flex flex-col  p-[4px] rounded-[4px] justify-end items-center gap-[16px] '>
              <div onMouseEnter={() => handleMouseEnter(index,'lessson')} onMouseLeave={()=> handleMouseLeave('lessson')}  className='flex w-[100%] h-[28px] justify-between items-center  cursor-pointer rounded-[4px] hover:bg-gray-input'>
                <div onClick={() => handleItemClick(index)} className='flex  h-[100%] items-center gap-[8px]'>
                  <div className={`w-[20px] h-[20px] rounded-[4px] ${item.color}`}></div>
                  <div className='font-iran-yekan text-[16px]'>{item.title}</div>
                </div>
                {
                hoveredIndex === index &&
              ...
              {expandedItems.includes(index) && item.projects && (
                <>
                  {item.projects && item.projects.map((project, projectIndex) => (
                    <div onMouseEnter={() => handleMouseEnter(projectIndex,'project', index)} onMouseLeave={()=> handleMouseLeave('project')} className='flex justify-between font-iran-yekan text-[16px] w-[100%] h-[100%] ' key={projectIndex}>
                      <div className='mr-[30px]'>{project}</div> 
                      {
                      hoveredIndex2 === projectIndex && index === hoveredIndex3 &&
                      <BiDotsHorizontalRounded className='m-[4px]' />
                      } 
                    </div>
                ...
                || expandedItems.includes(index) && !item.projects && (<Button
                  type='button'
                  disabled={false}
                  className=' h-[32px] p-[10px] text-green-button border-[2px] border-green-button font-iran-yekan text-[12px] gap-[4px] rounded-[6px] '
                  title='ساختن پروژه جدید'
